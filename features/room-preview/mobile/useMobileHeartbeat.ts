...
export interface MobileHeartbeatState {
  isConnected: boolean;
  failedCount: number;
  lastSuccessAt: number | null;
}

export function useMobileHeartbeat(
  sessionId: string,
  sessionStatus: string | null | undefined,
  disabled = false,
): MobileHeartbeatState {
  ...
  useEffect(() => {
    ...
    // Fire immediately on mount / status change, then on interval
    void ping();
  ...
  }, [disabled, sessionId, sessionStatus]);

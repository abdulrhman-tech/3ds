...
function useMobileBrowserLifecycle(sessionId: string, disabled = false) {
  useEffect(() => {
    if (disabled) return;
...
export default function MobileSessionClient({
  ...
  useMobileBrowserLifecycle(sessionId, devPreviewMode);
  ...
  const {
    ...
    localProductId,
    heartbeatConnected,
  } = useMobileSession({ devPreviewMode, products, sessionId });

...
export function Carousel3D({ images = IMAGES }: { images?: string[] }) {
  ...
  useEffect(() => {
    if (active >= carouselImages.length) setActive(0);
  }, [active, carouselImages.length]);

  /* ── Extract dominant color whenever active slide changes ────────────── */
  useEffect(() => {
    const activeImage = carouselImages[active];
  ...
  }, [active, ambR, ambG, ambB, carouselImages]);
  ...
  useEffect(() => {
    ...
    extractColor(carouselImages[0]).then(([r, g, b]) => {
      ambR.set(r); ambG.set(g); ambB.set(b);
    });
  }, [ambR, ambG, ambB, carouselImages]);

  /* ── Circular offset (shortest path) ────────────────────────────────── */
  const resolveOffset = (i: number) => {
    let off  = i - active;
    const half = Math.floor(carouselImages.length / 2);
    if (off >  half) off -= carouselImages.length;
    if (off < -half) off += carouselImages.length;
    return off;
  };
